...
-- (==)
-- 컬러명이 테이블 겹칠 경우 컬럼명 앞에 테이블명. 을 붙여야함.
...
-- 부서 정보(department_id, department_name, manager_id
-- 부서의 매니저 정보(frist_name, last_name, email)를 함께 출력하시오

select d.department_id, d.department_name, d.manager_id, e.first_name, e.last_name, e.email
...
select e.employee_id, e.last_name, e.email, e.phone_number, d.department_name, l.city, l.street_address
from employees e join departments d
on e.department_id = d.department_id
...
-- (예제6)employees 테이블에서 사원의 정보(employee_id, last_name, manager_id)와
-- 사원의 매니저 이름(last_name)을 함께 출력하시오

select e1.employee_id, e1.last_name as emp_name, e1.manager_id, e2.last_name as mgr_name
from employees e1 join employees e2
on e1.manager_id = e2.employee_id
order by e1.employee_id;

-- (예제7) employees, departments 테이블을 사용해서
-- 사원정보(employee_id, last_name, salary, hire_date)와alter
...
d.department_name, d.manager_id, m.last_name
from employees e join departments d
on e.department_id = d.department_id
join employees m
...
-- on절 join(기본조인)       left outer join
-- null 값은 출력이 안됨.	  right outer join
-- [inner join 예제]       
...
on e.department_id = d.department_id
order by e.employee_id;
-- emp : 부서가 없는 신입사원 출력 안됨
-- dept : 소속된 직원이 없는 빈 부서 출력 안됨
...
select o.order_num, o.order_date, o.member_id, m.member_name, m.phone, m.address
...
-- 출력 형식 : order_num, member_id, prod_id, prod_name, price, order_date

select o.order_num, o.member_id, p.prod_id, p.prod_name, p.price, o.order_date
from orders o join products p
on o.prod_id = p.prod_id;

select o.order_num, o.member_id, o.prod_id, p.prod_name, p.price, o.order_date
from orders o join products p
on o.prod_id = p.prod_id
order by order_num;
...
-- 주문한 제품의 정보(prod_name, price, company)을 함께 출력하시오alter
-- 출력 형식 : order_num, order_date, member_name, phone, address,
--           prod_name, price, company

select o.order_num, o.order_date, m.member_name, m.phone, m.address, p.prod_name, p.price, p.company
from orders o join members m
...
-- 회원이 주문한 정보(order_num, order_date, prod_id)와
-- 주문한 상품 정보(prod_name)를 함께 출력하되 주문한 적이 없는 회원도 모두 출력하시오.
-- 출력 형식 : member_id, member_name, phone, address,
--           order_num, order_date, prod_id, prod_name

select m.member_id, m.member_name, m.phone, m.address, o.order_num, o.order_date, p.prod_id, p.prod_name
